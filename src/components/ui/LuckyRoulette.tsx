  ...
  return (
    <div className="flex flex-col items-center gap-12 w-full max-w-2xl mx-auto">
      <div className="relative w-full max-w-[450px] aspect-square flex items-center justify-center">
        ...
        <div className={`absolute inset-4 rounded-full transition-all duration-1000 ${isSpinning ? 'bg-brand-orange/20 blur-[80px]' : 'bg-black/20 blur-[40px]'}`} />
        ...
        <div className="absolute inset-0 rounded-full bg-gradient-to-br from-[#FFD700] via-[#B8860B] to-[#8B4513] p-1 shadow-[0_20px_50px_rgba(0,0,0,0.5)] z-0">
          <div className="w-full h-full rounded-full bg-brand-dark flex items-center justify-center p-4 relative overflow-hidden">
            {/* Inner Gold Border */}
            <div className="absolute inset-2 rounded-full border-[6px] border-[#DAA520] opacity-30" />
            
            {/* The Light Bulbs */}
            {Array.from({ length: 24 }).map((_, i) => (
              <div 
                key={i}
                className={`absolute w-3 h-3 rounded-full z-20 transition-all duration-300 ${isSpinning ? (i % 2 === (Math.floor(Date.now()/200)%2) ? 'bg-white shadow-[0_0_15px_#fff,0_0_30px_#FFD700]' : 'bg-[#FFD700]/30') : 'bg-white shadow-[0_0_10px_#fff]'}`}
                style={{
                  top: '50%',
                  left: '50%',
                  transform: `rotate(${i * 15}deg) translateY(-210px) translateX(-50%)`,
                }}
              />
            ))}
          </div>
        ...
        <div className="absolute -top-6 left-1/2 -translate-x-1/2 z-50 flex flex-col items-center">
          <div className="w-10 h-14 bg-gradient-to-b from-[#FFD700] to-[#B8860B] rounded-t-full rounded-b-lg shadow-2xl relative flex items-center justify-center border-t border-white/40">
             <div className="w-4 h-4 bg-white rounded-full shadow-[0_0_15px_#fff] animate-pulse" />
             <div className="absolute -bottom-2 left-1/2 -translate-x-1/2 w-0 h-0 border-l-[12px] border-l-transparent border-r-[12px] border-r-transparent border-t-[16px] border-t-[#B8860B]" />
          </div>
        </div>

        {/* The Spinning Core */}
        <div className="relative w-[85%] h-[85%] z-10 p-2">
          <motion.div 
            animate={controls}
            initial={{ rotate: 0 }}
            className="w-full h-full rounded-full overflow-hidden shadow-[inset_0_0_50px_rgba(0,0,0,0.8),0_10px_30px_rgba(0,0,0,0.5)] border-4 border-[#FFD700]"
          >
          ...
          {/* Center Hub */}
          <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-28 h-28 z-30">
            <div className="w-full h-full rounded-full bg-gradient-to-br from-[#FFD700] via-[#DAA520] to-[#8B4513] p-1.5 shadow-[0_10px_20px_rgba(0,0,0,0.4)]">
              <div className="w-full h-full rounded-full bg-brand-dark border-2 border-[#FFD700]/30 flex items-center justify-center overflow-hidden relative">
                 ...
                 <div className={`w-14 h-14 rounded-full border-2 border-[#FFD700] flex items-center justify-center transition-all duration-700 ${isSpinning ? 'scale-110 shadow-[0_0_30px_#FFD700]' : 'scale-100'}`}>
                    <Ticket className="w-7 h-7 text-[#FFD700]" strokeWidth={2.5} />
                 </div>
              </div>
            </div>
          </div>
