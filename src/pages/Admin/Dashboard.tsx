          ...
          <div className="space-y-6 flex-1">
           ...
           {section.title === '04. Configurações de Doação Globais' && (
             <div className="mt-16 pt-16 border-t border-white/10">
               <h3 className="text-2xl font-serif italic mb-8">Gerenciador de Frases (Barra de Links)</h3>
               <TickerManager />
             </div>
           )}
         </div>
...
function TickerManager() {
  const { phrases, loading, addPhrase, deletePhrase, updatePhrase } = useTicker();
  ...
  return (
    <div className="space-y-8">
      <div className="bg-white/5 p-8 space-y-6 border border-white/10">
        <div className="space-y-3">
          <label className="text-[10px] uppercase tracking-widest text-white/40 font-bold">Nova Frase para a Barra</label>
          <div className="flex gap-4">
            <input 
              type="text" 
              value={newText}
              onChange={(e) => setNewText(e.target.value)}
              placeholder="Ex: Dança que Transforma Vidas..."
              className="flex-1 bg-white/5 border border-white/10 p-4 text-sm font-serif outline-none focus:border-brand-orange transition-all placeholder:opacity-20"
            />
            <button 
              onClick={handleAdd}
              disabled={isAdding || !newText.trim()}
              className="bg-brand-orange hover:bg-white hover:text-brand-dark px-8 text-[10px] uppercase font-bold tracking-widest transition-all flex items-center justify-center gap-3"
            >
      ...
      <div className="grid grid-cols-1 gap-3">
        <h5 className="text-[10px] uppercase tracking-widest text-white/20 font-bold mb-2">Frases Ativas</h5>
        {phrases.map((phrase) => (
          <div key={phrase.id} className="flex items-center justify-between p-6 bg-white/5 border border-white/5 group hover:border-white/20 transition-all">
            {editingId === phrase.id ? (
              <div className="flex-1 flex gap-4">
                <input 
                  ...
                  value={editText}
                  onChange={(e) => setEditText(e.target.value)}
                  className="flex-1 bg-brand-dark border border-white/10 p-3 text-sm font-serif outline-none focus:border-brand-orange"
                  autoFocus
                />
                <div className="flex gap-2">
                  <button onClick={() => handleUpdate(phrase.id)} className="px-4 py-2 bg-green-600 hover:bg-green-700 text-[10px] uppercase font-bold transition-colors">Salvar</button>
                  <button onClick={() => setEditingId(null)} className="px-4 py-2 bg-white/10 hover:bg-white/20 text-[10px] uppercase font-bold transition-colors">Sair</button>
                </div>
              </div>
            ) : (
              <>
                <span className="font-serif text-lg text-white/80 italic">\"{phrase.text}\"</span>
                <div className="flex gap-2">
                  <button 
