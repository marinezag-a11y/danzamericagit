                    ...
                    {section.title === '01. Configurações Globais' && (
                      <div className="border-t border-white/5 pt-12 space-y-12">
                        ...
                        <div className="bg-black/20 border border-white/5 p-8 rounded-2xl shadow-inner relative overflow-hidden group">
                          ...
                          {/* InfinitePay Pix Config Card */}
                          <div className="mt-12 bg-white/[0.02] border border-brand-orange/20 p-8 rounded-2xl ml-4">
                            <h5 className="text-[10px] uppercase tracking-[0.3em] text-brand-orange font-bold mb-2">Dados Pix InfinitePay (Checkout)</h5>
                            <p className="text-[9px] text-white/30 mb-8 leading-relaxed">
                              Configure a chave Pix registrada na sua conta InfinitePay. Esta chave é usada para gerar o Pix Copia e Cola e o QR Code no checkout. Encontre-a em: App InfinitePay → Conta → Chaves Pix.
                            </p>
                            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                              <div className="space-y-4">
                                <label className="block text-[10px] uppercase tracking-[0.3em] text-brand-orange font-bold ml-1">Chave Pix InfinitePay</label>
                                <input 
                                  type="text" 
                                  className="w-full bg-black/40 border border-white/10 p-4 text-sm font-sans focus:border-brand-orange/50 outline-none transition-all text-white/80 rounded-xl"
                                  value={localValues['infinitepay_pix_key'] || ''}
                                  onChange={(e) => setLocalValues(prev => ({ ...prev, infinitepay_pix_key: e.target.value }))}
                                  placeholder="Ex: CNPJ, e-mail ou chave aleatória da InfinitePay"
                                />
                              </div>
                              <div className="space-y-4">
                                <label className="block text-[10px] uppercase tracking-[0.3em] text-brand-orange font-bold ml-1">Nome do Recebedor (InfinitePay)</label>
                                <input 
                                  type="text" 
                                  className="w-full bg-black/40 border border-white/10 p-4 text-sm font-sans focus:border-brand-orange/50 outline-none transition-all text-white/80 rounded-xl"
                                  value={localValues['infinitepay_receiver'] || ''}
                                  onChange={(e) => setLocalValues(prev => ({ ...prev, infinitepay_receiver: e.target.value }))}
                                  placeholder="Ex: NUCLEO DE DANCA TATIANA FIGUEIREDO"
                                />
                              </div>
                            </div>
                          </div>
                          <div className="flex justify-end mt-8 pl-4">
                            <button 
                              onClick={() => handleSaveSection('Links de Doação', ['pix_key', 'vakinha_url', 'contact_whatsapp', 'dancers_count', 'infinitepay_pix_key', 'infinitepay_receiver'])}
