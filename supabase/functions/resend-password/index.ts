...
serve(async (req) => {
  ...
  try {
    ...
    if (authError || !user) throw new Error(`Não autorizado. Detalhes: ${authError?.message || 'Usuário não encontrado'}`)
      ...
      const welcomeEmailHtml = `
        <div style="font-family: serif; background-color: #1A1A1A; color: white; padding: 40px; border: 1px solid #FF5A1F; max-width: 600px; margin: auto;">
          ...
          <div style="text-align: center; margin: 40px 0;">
            <a href="https://nucleotatianafigueiredo.com.br/admin" style="background-color: #FF5A1F; color: white; padding: 18px 35px; text-decoration: none; font-weight: bold; text-transform: uppercase; letter-spacing: 2px; font-size: 11px; display: inline-block; border-radius: 2px;">Acessar Painel Agora</a>
          </div>
